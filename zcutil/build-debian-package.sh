...
PACKAGE_VERSION=$($SRC_PATH/src/asofed --version | grep version | cut -d' ' -f4 | tr -d v)
...
# Copy docs
cp $SRC_PATH/doc/release-notes/release-notes-1.0.14.md $DEB_DOC/changelog
cp $SRC_DEB/changelog $DEB_DOC/changelog.Debian
cp $SRC_DEB/copyright $DEB_DOC
cp -r $SRC_DEB/examples $DEB_DOC
# Copy manpages
cp $SRC_DOC/man/asofed.1 $DEB_MAN
cp $SRC_DOC/man/asofe-cli.1 $DEB_MAN
cp $SRC_DOC/man/asofe-fetch-params.1 $DEB_MAN
# Copy bash completion files
cp $SRC_PATH/contrib/asofed.bash-completion $DEB_CMP/asofed
cp $SRC_PATH/contrib/asofe-cli.bash-completion $DEB_CMP/asofe-cli
# Gzip files
